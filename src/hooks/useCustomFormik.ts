...
const useCustomFormik = ({ onSubmit }: Props) => {
  ...
  const formik = useFormik<DTOPreferenceReq>({
    initialValues: { firstName: "", lastName: "", DNI: "", email : "", phone : "" },
    validationSchema: Yup.object({
      ...
      email : Yup.string().min(6, "Almenos 6 caracteres").max(70, "El maximo de caracteres es de 70").required("Campo requerido").email('Correo no valido'),
      phone : Yup.number().typeError("El campo debe ser numerico")
      .positive("El valor debe ser positivo")
      .integer("El valor debe ser entero")
    ...
    onSubmit:  ({ firstName, lastName, DNI, email, phone }) => {
    //   MySwal.showLoading();
    //   try {
